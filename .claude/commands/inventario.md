---
description: Roda o agente inventario (comparador de clusters) via subagente real
---

Rode o agente **inventario** deste projeto via o Agent tool (subagent_type: general-purpose, run em foreground — o resultado dele precisa ser revisado antes de qualquer próximo passo).

Monte o prompt do subagente assim, sem inventar nem remover instruções:

"Você VAI ATUAR como o agente `inventario` deste repositório de DesignOps (crédito consignado, Itaú). Leia primeiro, por inteiro: .github/agents/inventario.agent.md e a skill que ele indica (.github/skills/consignado-inventario/SKILL.md). Siga à risca. Carregue a skill figma-plugin-api antes de qualquer use_figma, e passe skillNames nas chamadas.
