...
const compositionDefinitions = {
  analista: { state: 'ANALISANDO', next: 'AGUARDANDO_APROVACAO_CONTRATO', packageFile: 'proposta-composicao-etapa.json', script: 'validateStageCompositionProposal.js', receipt: ['ANALISTA', 'ANALISE'], packageState: 'PRONTO_PARA_REVISAO' },
  montagem: { state: 'MONTANDO', next: 'VALIDANDO', packageFile: 'pacote-composicao-montagem.json', script: 'validateStageCompositionAssembly.js', receipt: ['MONTADOR', 'MONTAGEM'], packageState: 'CONCLUIDA_PARA_VALIDACAO' },
  veredito: { state: 'VALIDANDO', next: 'CONCLUIDA', packageFile: 'pacote-composicao-etapa.json', script: 'validateStageCompositionPackage.js', receipt: ['VALIDADOR', 'VALIDACAO'], packageState: 'CONCLUIDA' },
}[name]
const definitions = state?.tipoRodada === 'COMPOSICAO_ETAPA' ? compositionDefinitions : standardDefinitions
const failures = []
...
if (state.schemaVersion === 2 && state.tipoRodada === 'MOMENTO') {
  for (const modalidade of state.entrada.modalidades) {
    state.pacotesPorModalidade[modalidade][name] = { ...state.pacotes[name] }
  }
}
if (name === 'analista') {
...
if (name === 'veredito') {
  ...
  if (state.tipoRodada !== 'COMPOSICAO_ETAPA') state.checkpoints.promocao.status = 'AGUARDANDO_APROVACAO'
}
