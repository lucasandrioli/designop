...
variabilizador manda ler `docs/clusters/<cluster>.md`; o validador
confere as regras ATIVAS desse manual; o inventario cruza cada
...
| 2.5 | Mapa de fluxo | docs/mapa-fluxo-<escopo>.md | pode ser rascunho; o inventario gera do prototipo (Modo Fluxos) |

Regra de ouro da camada 2: **regra que nao esta escrita, o agente nao
...
## Camada 3 — ARTEFATO FIGMA (bloqueia por etapa)

Dono: voce. E o material concreto no arquivo.

| # | Item | Minimo para o CMV |
| --- | --- | --- |
...
13 e 14): o inventario audita e reporta, a normalizacao e trabalho do
...
(Camada 3 do projeto, ainda nao destravada).
...
## Ordem de execucao (dia 0 em diante)
    ...
    escolher 1 etapa + 2 clusters
        v
    Camada 2 minima (manual dos 2 clusters, doc da etapa)  <-- so o humano faz
        v
    Camada 3 minima (arquivo, libs, collection, 2 telas cruas)  <-- so o designer faz
        v
    /inventario  -> checklist de aprovacao
        v
    VOCE aprova o schema (checkpoint obrigatorio, nao pule)
        v
    /variabilizador -> template bindado + carimbo
        v
...
O Passo 0 do inventario (skill `consignado-inventario`) verifica os
pre-requisitos antes de comparar qualquer coisa, e RECUSA comecar se
faltar item da camada 2 ou 3 — devolvendo a lista do que falta em
linguagem de negocio, nao em nome de arquivo. Isso existe porque
documento sozinho nao e cumprido: quem cobra e o agente.

O que os agentes NAO checam: camada 1. Ambiente e responsabilidade
